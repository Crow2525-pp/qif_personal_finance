...
WITH transaction_base AS (
    SELECT
        ft.transaction_date,
        CASE
            WHEN ft.transaction_amount < 0 THEN -ft.transaction_amount
...
amazon_transactions AS (
    SELECT
        transaction_date,
        abs_amount,
        DATE_TRUNC('month', transaction_date) AS year_month,
        CASE
            WHEN LOWER(transaction_text) LIKE '%prime%'
                OR LOWER(transaction_text) LIKE '%subscribe%'
                OR LOWER(transaction_text) LIKE '%recurring%'
                OR LOWER(transaction_text) LIKE '%membership%'
            THEN 'Subscription/Recurring'
            ELSE 'One-Off Purchase'
        END AS purchase_type
    FROM transaction_base
    WHERE
        level_3_store = 'Amazon'
        OR LOWER(transaction_text) LIKE '%amazon%'
        OR LOWER(transaction_text) LIKE '%amzn%'
),
            ...
            WHEN LAG(average_order_value, 1) OVER (ORDER BY year_month) > 0 THEN
                ROUND(
                    (
                        (
                            average_order_value -
                            LAG(average_order_value, 1) OVER (ORDER BY year_month)
                        ) /
                        LAG(average_order_value, 1) OVER (ORDER BY year_month)
                    ) * 100,
                    1
...
SELECT
    ...
    (
        SELECT largest_order_value
        FROM period_stats ps
        WHERE ps.year_month = bst.year_month
    ...
    (
        SELECT SUM(
            CASE
                WHEN pt.purchase_type = 'Subscription/Recurring' THEN pt.category_spend
                ELSE 0
            END
        )
        FROM purchase_type_split pt
        WHERE pt.year_month = bst.year_month
    ) AS recurring_spend,
    (
        SELECT SUM(
            CASE
                WHEN pt.purchase_type = 'One-Off Purchase' THEN pt.category_spend
                ELSE 0
            END
        )
        FROM purchase_type_split pt
    ...
    (
        SELECT SUM(
            CASE
                WHEN pt.purchase_type = 'Subscription/Recurring' THEN pt.order_count
                ELSE 0
            END
        )
        FROM purchase_type_split pt
    ...
    (
        SELECT SUM(
            CASE
                WHEN pt.purchase_type = 'One-Off Purchase' THEN pt.order_count
                ELSE 0
            END
        )
        FROM purchase_type_split pt
