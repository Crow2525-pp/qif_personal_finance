...
[
    ...
    },
    {
        ...
        "effort":  "medium",
        "status":  "completed",
        "notes":  "Completed 2026-02-08: refactored payoff panels to mortgage-tagged debt model (`rpt_debt_reduction_tracking`, `is_mortgage=TRUE`), documented assumptions in dashboard guide, and updated sensitivity to use blended interest rate + amortization-based payoff math."
    },
    {
        "id":  89,
        ...
        "effort":  "small",
        "status":  "completed",
        "notes":  "Completed 2026-02-08: replaced static budget target with a 12-month dynamic baseline in Postgres query logic, normalized dashboard currency units to AUD, and repurposed duplicated trend panels into distinct retailer, total, and rolling-average views."
    },
    {
        "id":  90,
    ...
    },
    {
        ...
        "scope":  "grafana/provisioning/dashboards/03-monthly-budget-summary.json; panel id 11 (`Budget Adjustments for Next Month`); reporting.rpt_budget_cockpit_adjustment_suggestions",
        "description":  "Checker run on 2026-02-08 shows panel 11 returns `empty (0 rows)` and fails smoke checks. Add safe fallback output when no adjustment rows exist (e.g., `No budget adjustments required this month`) or repair upstream model conditions so panel consistently returns a rowset.",
        "category":  "dashboard-fix",
        "effort":  "small",
        "status":  "pending",
        "notes":  "Found via `scripts/check_grafana_dashboards.py --dashboard-range 1-23 --schema-validate` on 2026-02-08."
    }
]
