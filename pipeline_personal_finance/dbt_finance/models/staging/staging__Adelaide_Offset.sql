...
WITH parsed_memo_data AS (
    ...
    FROM {{ source('personalfinance_dagster', 'Adelaide_Offset_Transactions') }}
)

SELECT
    -- Extract date component from the original 'date' field
...
FROM {{ source('personalfinance_dagster', 'Adelaide_Offset_Transactions') }} AS transactions
