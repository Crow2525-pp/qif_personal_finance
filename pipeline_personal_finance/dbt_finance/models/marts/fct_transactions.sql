{{
  config(
    ...
    post_hook=[
      "{{ create_fk_if_not_exists(this, 'account_key', ref('dim_accounts'), 'account_key', 'fk_fct_txn_account') }}",
      "{{ create_fk_if_not_exists(this, 'category_key', ref('dim_categories'), 'category_key', 'fk_fct_txn_category') }}"
    ]
  )
}}
