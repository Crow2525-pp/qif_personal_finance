...
-- Test 1: fact_transactions_enhanced.account_key references valid dim_accounts_enhanced.account_key
...
-- Test 2: fact_transactions_enhanced.category_key references valid dim_categories_enhanced.category_key  
...
-- Test 3: fact_daily_balances.account_key references valid dim_accounts_enhanced.account_key
invalid_daily_balance_references AS (
  SELECT 
    fdb.account_key,
    fdb.balance_date,
    'Missing account reference in daily balances' AS error_type
  FROM {{ ref('fact_daily_balances') }} fdb
  LEFT JOIN {{ ref('dim_accounts_enhanced') }} da
